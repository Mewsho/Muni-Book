...
mongoose.connect(
  "mongodb+srv://FcoTorres:hEqGLg4XvhwgO9y5@cluster0.45avn.mongodb.net/BaseBiblioteca",
...
const typeDefs = gql`
  ...
  type Query {
    ...
    getDetalleSolicitudPrestamosByIdEjemplares(
      id: ID!
    ): DetalleSolicitudPrestamo
    getDetalleSolicitudPrestamosByIdSolicitudPrestamo(
      id: ID!
    ): DetalleSolicitudPrestamo
    getDetalleSolicitudPrestamosByIdEjemplaresSolicitudPrestamo(
      id: ID!
    ): DetalleSolicitudPrestamo
    ...
    getDocumentosByTituloAndAutorAndTipoAndCategoria(
      tipoId: ID
      categoriaId: ID
      titulo: String
      autor: String
    ...
    getUsuarioByCorreoAndCheckPassword(
      correo: String!
      password: String!
    ): Usuario
    sendEmail(correo: String, codigoVerificador: Int, nombres: String): Boolean
    getNEjemplaresDisponiblesByDocumentId(
      documentoId: ID
      cantEjemplares: Int
    ): [Ejemplar]
...
const resolvers = {
  Query: {
    ...
    async getSolicitudPrestamosUsuario(obj) {
      const solicitudPrestamos = await SolicitudPrestamo.find().populate(
        "usuario"
      );
      return solicitudPrestamos;
    },
    ...
    async getSolicitudPrestamoByIdUsuario(obj, { id }) {
      let solicitudPrestamo = await SolicitudPrestamo.findById(id).populate(
        "usuario"
      );
      return solicitudPrestamo;
    },
    ...
    async getSolicitudPrestamosPrestamos(obj) {
      let solicitudPrestamo = await SolicitudPrestamo.find().populate(
        "prestamos"
      );
      return solicitudPrestamo;
    },
    ...
    async getSolicitudPrestamosUsuarioPrestamos(obj) {
      let solicitudPrestamo = await SolicitudPrestamo.find()
        .populate("usuario")
        .populate("prestamos");
      return solicitudPrestamo;
    },
    ...
    async getSolicitudPrestamoByIdUsuarioPrestamos(obj) {
      let solicitudPrestamo = await SolicitudPrestamo.findById(id)
        .populate("usuario")
        .populate("prestamos");
      return solicitudPrestamo;
    },
    ...
    async getDocumentosByIdCategoria(obj, { id }) {
      let documentos = await Documento.findById(id).populate(
        "categoriaDocumento"
      );
      return documentos;
    },
    ...
    async getDocumentosByIdTipoCategoria(obj, { id }) {
      let documentos = await Documento.findById(id)
        .populate("tipoDocumento")
        .populate("categoriaDocumento");
      return documentos;
    },
    ...
    async getDetalleSolicitudPrestamosEjemplares(obj) {
      let detalleSolicitudPrestamos =
        await DetalleSolicitudPrestamo.find().populate("ejemplares");
      return detalleSolicitudPrestamos;
    },

    async getDetalleSolicitudPrestamosEjemplaresSolicitudPrestamo(obj) {
      let detalleSolicitudPrestamos =
        await DetalleSolicitudPrestamo.find().populate("solicitudPrestamo");
      return detalleSolicitudPrestamos;
    },
    ...
    async getDetalleSolicitudPrestamosEjemplaresSolicitudPrestamo(obj) {
      let detalleSolicitudPrestamos = await DetalleSolicitudPrestamo.find()
        .populate("ejemplares")
        .populate("solicitudPrestamo");
      return detalleSolicitudPrestamos;
    },

    async getDetalleSolicitudPrestamosById(obj, { id }) {
      let detalleSolicitudPrestamo = await DetalleSolicitudPrestamo.findById(
        id
      );
    ...
    async getDetalleSolicitudPrestamosByIdEjemplares(obj, { id }) {
      let detalleSolicitudPrestamos = await DetalleSolicitudPrestamo.findById(
        id
      ).populate("ejemplares");
      return detalleSolicitudPrestamos;
    },
    ...
    async getDetalleSolicitudPrestamosByIdEjemplaresSolicitudPrestamo(
      obj,
      { id }
    ) {
      let detalleSolicitudPrestamos = await DetalleSolicitudPrestamo.findById(
    ...
    async getDocumentosByTituloAndAutorAndTipoAndCategoria(
      obj,
      { tipoId, categoriaId, titulo, autor }
    ) {
      if (titulo == null) {
        titulo = "";
      }
      ...
      if (tipoId == null) {
        let documentos = await Documento.find({
          titulo: { $regex: titulo, $options: "i" },
          autor: { $regex: autor, $options: "i" },
          categoriaDocumento: { $in: [categoriaId] },
        })
          .populate("tipoDocumento")
          .populate("categoriaDocumento");
        return documentos;
      ...
      let documentos = await Documento.find({
        ...
        autor: { $regex: autor, $options: "i" },
        tipoDocumento: tipoId,
        categoriaDocumento: categoriaId,
      })
        .populate("tipoDocumento")
        .populate("categoriaDocumento");
      return documentos;
    ...
    async sendEmail(obj, { correo, codigoVerificador, nombres }) {
      ...
      try {
            ...
            Messages: [
              {
                ...
                HTMLPart: `<h3>Hola ${nombres}</h3><br />Bienvenido a Munibook, para completar la creación de tu cuenta, por favor usa el codigo de abajo:<br /><h2>${codigoVerificador}</h2><br />Por favor, ingresa el codigo en la siguiente <a href="ConfirmarCodigo.html">pagina</a><br />Gracias por registrarte a Munibook!`,
              },
            ],
          });
        ...
        console.error(
          `Error: ${err.message} - Código de estado: ${err.statusCode}`
        );
      }
    },

    async getNEjemplaresDisponiblesByDocumentId(
      obj,
      { documentoId, cantEjemplares }
    ) {
      let Ejemplares = await Ejemplar.find({
    ...
    async getSolicitudPrestamoByUsuarioId(obj, { usuarioID }) {
      let solicitudPrestamo = await SolicitudPrestamo.findOne({
        usuario: usuarioID,
      }).populate("prestamos");
      return solicitudPrestamo;
    },
  ...
  Mutation: {
    ...
    async updPrestamo(obj, { id, input }) {
      let ejemplarBus = await Ejemplar.findById(input.ejemplar);
      let prestamo = await Prestamo.findByIdAndUpdate(id, {
        tipoPrestamo: input.tipoPrestamo,
        ejemplar: ejemplarBus._id,
        fechaPrestamo: input.fechaPrestamo,
        fechaDevolucion: input.fechaDevolucion,
    ...
    async addDocumento(obj, { input }) {
      ...
      let documento = new Documento({
        ...
        anioSalida: input.anioSalida,
        edicion: input.edicion,
        codigo: input.codigo,
        tipoDocumento: tipoDocumentoBus._id,
        categoriaDocumento: inputCategoriasList,
      });
    ...
    async addEjemplar(obj, { input }) {
      ...
      let ejemplar = new Ejemplar({
        documento: documentoBus._id,
        estado: input.estado,
        ubicacion: input.ubicacion,
        estadoTexto: input.estadoTexto,
        codigo: input.codigo,
      });
      await ejemplar.save();
      return {
    ...
    async addDetalleSolicitudPrestamo(obj, { input }) {
      ...
      let solicitudPrestamoBus = await SolicitudPrestamo.findById(
        input.solicitudPrestamo
      );
      ...
      let detalleSolicitudPrestamo = new DetalleSolicitudPrestamo({
        ejemplares: inputEjemplarList,
        solicitudPrestamo: solicitudPrestamoBus._id,
      });
    ...
    async updDetalleSolicitudPrestamo(obj, { id, input }) {
      ...
      let detalleSolicitudPrestamo =
        await DetalleSolicitudPrestamo.findByIdAndUpdate(id, {
          ejemplares: ejemplarIdFinalList,
          solicitudPrestamo: solicitudPrestamoBus._id,
        });
      return {
        statusCode: "200",
    ...
    async addSolicitudPrestamo(obj, { input }) {
      ...
      let solicitudPrestamo = new SolicitudPrestamo({
        usuario: usuarioBus._id,
        fechaSolicitud: input.fechaSolicitud,
        prestamos: ListaFinalPrestamos,
    ...
    async updCategoriaDocumento(obj, { input }) {
      let categoriaDocumento = await CategoriaDocumento.findByIdAndUpdate(
        id,
        input
      );
      return {
